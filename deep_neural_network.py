...
neurons = tuple([np.zeros((y,1)) for y in layer_sizes])

# Weights
# N-1 matrices of size  n_L-1 x n_L (skipping first layer)
weights = tuple([np.zeros((y,x)) for x,y in zip(layer_sizes[:-1],layer_sizes[1:])])

# Biases
# N-1 arrays of size 1 x n_L (skipping first layer)
biases = tuple([np.zeros((y,1)) for y in layer_sizes[1:]])
...
def forward_propagate(X):
    ...
    neurons[0][True] = np.transpose(np.matrix(X).flatten())

    # Go through every layer and calculate neuron activation
    for i in range(len(neurons)-1):
        neurons[i+1][True] = np.matmul(weights[i], sigmoid(neurons[i])) + biases[i]

    return sigmoid(neurons[-1])
...
def backpropagate(Xs, Ys, alpha):
    ...
    while Xs:
        ...
        dCdW = tuple([np.zeros((y,x)) for x,y in zip(layer_sizes[:-1],layer_sizes[1:])])
        dCdB = tuple([np.zeros((y,1)) for y in layer_sizes[1:]])
        ...
        for i in reversed(range(len(layer_sizes)-1)):
            ...
            dZdALm1 = weights[i] # nLxnL-1 TODO: may have to transpose
            dCdZLx = np.matmul(np.transpose(dZdALm1),dCdZLx) # nL-1x1
    ...
    dCdB = tuple([np.mean(x,axis=0) for x in zip(*dCdBs)])
    C = np.mean(costs)
    
    # Adjust weights and biases
    for W,dW in zip(weights,dCdW):
        W[True] = W - alpha*dW
    for B,dB in zip(biases,dCdB):
        B[True] = B - alpha*dB
...
def train():
    ...
    biases = tuple([np.random.randn(*b.shape) for b in biases])
